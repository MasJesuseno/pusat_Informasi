    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in zoom-in-95 duration-200">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Delete Question Group</h3>
        ...
        <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3 mb-6">
          <p className="text-sm text-red-700">
            All questions in this group will also be deleted.
          </p>
        </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${colors[status] || "bg-gray-50 text-gray-600 border-gray-200"}`}>
      <span className={`w-1.5 h-1.5 rounded-full ${status === "PUBLIC" ? "bg-emerald-500" : status === "INTERNAL" ? "bg-amber-500" : "bg-gray-400"}`} />
      {status === "PUBLIC" ? "Public" : status === "INTERNAL" ? "Internal" : "Draft"}
    </span>
  );
}
...
export default function EditQuestionGroupPage() {
  ...
  useEffect(() => {
    fetch(`/api/question-groups/${id}?locale=en`)
      .then((r) => r.json())
      .then((data) => {
        if (data.group) {
          setFormData({
            nameEn: data.group.nameEn || "",
            nameId: data.group.nameId || "",
            order: data.group.order || 0,
      ...
      .catch(() => {
        setToast({ message: "Failed to load question group", type: "error" });
        setFetching(false);
      });
    fetch("/api/settings")
      .then(r => r.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setToast({ message: "Question group updated successfully!", type: "success" });
        setTimeout(() => router.push("/enter/question-groups"), 600);
      } else {
        const data = await res.json();
        setToast({ message: data.error || "Failed to update question group", type: "error" });
      }
    } catch {
  ...
  const handleDeleteConfirm = async () => {
    setDeleting(true);
    try {
      const res = await fetch(`/api/question-groups/${id}`, { method: "DELETE" });
      if (res.ok) {
        setDeleteModalOpen(false);
        setToast({ message: "Question group deleted successfully", type: "success" });
        setTimeout(() => router.push("/enter/question-groups"), 600);
      } else {
        const data = await res.json();
        setToast({ message: data.error || "Failed to delete question group", type: "error" });
        setDeleting(false);
        setDeleteModalOpen(false);
      }
    } catch {
      setToast({ message: "Failed to delete question group", type: "error" });
      setDeleting(false);
      setDeleteModalOpen(false);
    }
  };
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          <Link href="/enter/question-groups" className="hover:text-indigo-600 transition-colors flex items-center gap-1.5">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <span className="text-gray-900 font-medium">Edit Question Group</span>
        </nav>
        ...
        <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-white/15 flex items-center justify-center">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2-2 2 2m0 0l2 2-2 2m-2-2l-2-2 2-2m6 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <div>
                <h1 className="text-lg font-semibold">Edit Question Group</h1>
            ...
            <button
              ...
              aria-label="Delete this question group"
          ...
          <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 space-y-7">
            ...
            {/* Status & Order */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {/* Status Toggle — accessible radiogroup with 3 options */}
              <fieldset>
                ...
                <div className="inline-flex bg-gray-100 rounded-xl p-1 gap-0.5 border border-gray-200" role="radiogroup" aria-label="Visibility">
                  ...
                  <button
                    type="button"
                    role="radio"
                    aria-checked={formData.status === "DRAFT"}
                  ...
                  >
                    <span className={`w-2 h-2 rounded-full ${formData.status === "DRAFT" ? "bg-gray-400" : "bg-gray-300"}`} />
                    Draft
                  </button>
                </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                type="button"
                onClick={() => router.push("/enter/question-groups")}
