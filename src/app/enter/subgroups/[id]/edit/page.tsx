    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-in zoom-in-95 duration-200">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Delete Sub Group</h3>
        ...
        <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3 mb-6">
          <p className="text-sm text-red-700">
            All articles in this sub group will remain but will no longer be associated with it.
          </p>
        </div>
...
export default function EditSubGroupPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [groupsRes, sgRes] = await Promise.all([
          fetch("/api/groups?locale=en"),
          fetch(`/api/subgroups/${id}`),
        ]);

        if (!sgRes.ok) throw new Error("Failed to fetch subgroup");
        ...
        if (sgData.subgroup) {
          setFormData({
            nameEn: sgData.subgroup.translations?.find((t: any) => t.locale === "en")?.name || "",
            nameId: sgData.subgroup.translations?.find((t: any) => t.locale === "id")?.name || "",
            groupId: String(sgData.subgroup.groupId ?? ""),
            order: String(sgData.subgroup.order ?? 0),
            status: sgData.subgroup.status || "PUBLIC",
          });
        }
      } catch {
        setToast({ message: "Failed to load subgroup data", type: "error" });
      } finally {
        setFetching(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/subgroups/${id}`, {
        method: "PUT",
      ...
      if (res.ok) {
        setToast({ message: "Sub group updated successfully!", type: "success" });
        setTimeout(() => router.push("/enter/subgroups"), 600);
      } else {
        const data = await res.json();
        setToast({ message: data.error || "Failed to update subgroup", type: "error" });
      }
    } catch {
  ...
  const handleDeleteConfirm = async () => {
    setDeleting(true);
    try {
      const res = await fetch(`/api/subgroups/${id}`, { method: "DELETE" });
      if (res.ok) {
        setDeleteModalOpen(false);
        setToast({ message: "Sub group deleted successfully", type: "success" });
        setTimeout(() => router.push("/enter/subgroups"), 600);
      } else {
        const data = await res.json();
        setToast({ message: data.error || "Failed to delete subgroup", type: "error" });
        setDeleting(false);
        setDeleteModalOpen(false);
      }
    } catch {
      setToast({ message: "Failed to delete subgroup", type: "error" });
      setDeleting(false);
      setDeleteModalOpen(false);
    }
  };
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          <Link href="/enter/subgroups" className="hover:text-indigo-600 transition-colors flex items-center gap-1.5">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <span className="text-gray-900 font-medium">Edit Sub Group</span>
        </nav>
        ...
        <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-semibold">Edit Sub Group</h1>
            ...
            <button
              ...
              aria-label="Delete this sub group"
          ...
          <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-8 space-y-7">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                Parent Group <span className="text-red-400">*</span>
              </label>
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-shadow appearance-none"
                  required
                >
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                type="button"
                onClick={() => router.push("/enter/subgroups")}
