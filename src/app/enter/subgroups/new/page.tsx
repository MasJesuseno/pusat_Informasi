...
export default function NewSubGroupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setToast({ message: "Sub group created successfully!", type: "success" });
        setTimeout(() => router.push("/enter/subgroups"), 600);
      } else {
        const data = await res.json();
        setToast({ message: data.error || "Failed to create subgroup", type: "error" });
      }
    } catch {
      setToast({ message: "An error occurred while creating", type: "error" });
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <span className="text-gray-900 font-medium">Create Sub Group</span>
        </nav>
        ...
        <div className="rounded-xl border border-gray-200 overflow-hidden shadow-sm">
          ...
          >
            ...
            <div>
              <h1 className="text-lg font-semibold">Create Sub Group</h1>
              <p className="text-sm text-white/70">Add a new sub group to organize articles</p>
            </div>
          </div>
