...
export default function EditQuestionPage() {
      ...
      .then((data) => {
        if (data.question) {
          ...
          if (q.options?.length === 4) {
            setOptions(q.options.map((o: any) => o.content));
          }
          // Set editor content after data loads
          if (editor) {
            editor.commands.setContent(q.content || "", { emitUpdate: false });
          }
        }
        setFetching(false);
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) router.push(`/enter/questions?questionGroupId=${questionGroupId}`);
    } catch {
              ...
              <div className="border border-gray-300 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-transparent">
                ...
                <div className="flex flex-wrap items-center gap-0.5 px-2 py-1.5 bg-gray-50 border-b border-gray-200">
                  ...
                  <ToolBtn onClick={handleImageUpload} title="Insert Image in Question">
                    {uploading ? <span className="text-xs">...</span> : <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>}
                  </ToolBtn>
                </div>
